...
// Variable for the highest dB SPL value
float maxdBspl = 0.0; 
// variable to check whether the start delay has expired. 
bool isDelayOver = false; 

// Constants for dB SPL conversion
// Reference voltage of the microcontroller (in volts)
const float Vref = 3.3;     
// RMS voltage corresponding to 94 dB SPL
const float Vrms = 0.0036;  
// Microphone sensitivity (in dBV)
...
// Time in milliseconds to average the values
const unsigned long averagingTime = 200;  
...
void HandleInput() {
  ...
  if (doNextStep)
  {
    if (isMeasuring) 
    {
      ...
      absoluteSum = 0.0;
      absoluteCount = 0; 
	  // Resetting the highest dB SPL value at the start of a new measurement
      maxdBspl = 0.0; 
	  // Reset start delay
      isDelayOver = false; 
	  // Record start time
    ...
    else 
    {
      ...
      {
		// Display of the highest dB SPL value at the end of the measurement
        ShowMaxdBspl(); 
        redButtonResult = 1;
      }
      else
...
void ShowResult() {
  ...
  if (isDelayOver && dBspl > maxdBspl) { 
    // Check for the highest dB SPL value after the start delay
    maxdBspl = dBspl;
  }
}
...
void ShowdBValue(String title, float dbSpl) {
  ...
  {
    display.println("Measurement too short");
  }
