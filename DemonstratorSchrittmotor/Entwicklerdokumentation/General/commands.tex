...
\definecolor{yamlblue}{RGB}{0, 100, 204}  % Blue color for the YAML syntax
\definecolor{yamlbrown}{RGB}{165, 42, 42} % Brown color for text after colon
...
\lstdefinelanguage{yaml}{
	basicstyle=\ttfamily\color{yamlblue},  % Set all text in blue
	morestring=[b]",
	sensitive=false,
	morecomment=[l]{\#},
	commentstyle=\color{gray},
	stringstyle=\color{yamlbrown},  % Default color for strings after colon
	showstringspaces=false,
	breaklines=true,
	frame=single,
	tabsize=2,
	moredelim=[l][\color{yamlbrown}]{:},  % Color text after the first colon in brown
	literate=
	{true}{{\color{yamlblue}true}}1
	{false}{{\color{yamlblue}false}}1
}
...
\newcommand\yamlstyle{
	\lstset{ 
		backgroundcolor=\color{white},   % choose the background color; you must add \usepackage{color} or \usepackage{xcolor}; should come as last argument
		basicstyle=\footnotesize\color{yamlblue},        % the size of the fonts that are used for the code
		breakatwhitespace=false,         % sets if automatic breaks should only happen at whitespace
		breaklines=true,                 % sets automatic line breaking
		captionpos=b,                    % sets the caption-position to bottom
		commentstyle=\color{mygreen},    % comment style
...
\lstdefinelanguage{PowerShell}[]{command.com}{}

%   Arduino-Programs
\newcommand\PowerShellStyle{
	\lstset{ 
		backgroundcolor=\color{white},   % choose the background color; you must add \usepackage{color} or \usepackage{xcolor}; should come as last argument
		basicstyle=\footnotesize,        % the size of the fonts that are used for the code
		breakatwhitespace=false,         % sets if automatic breaks should only happen at whitespace
...
%        morecomment=[l]{::},
		...
		keepspaces=true,                 % keeps spaces in text, useful for keeping indentation of code (possibly needs columns=flexible)
		keywordstyle=\color{blue},       % keyword style
		language=command.com,            % the language of the code
        morekeywords={arduino-cli, arduino:mbed_portenta, arduino} % if you want to add more keywords to the set
		numbers=left,                    % where to put the line-numbers; possible values are (none, left, right)
		numbersep=5pt,                   % how far the line-numbers are from the code
		numberstyle=\tiny\color{mygray}, % the style that is used for the line-numbers
		rulecolor=\color{black},         % if not set, the frame-color may be changed on line-breaks within not-black text (e.g. comments (green here))
...
%Arduino for external files
\newcommand\PowerShellExternal[2]{{
		\PowerShellStyle
		\lstinputlisting[#1]{#2}}}
